...
def most_important_value_topic(new_dict):
    ...
    for topic in new_dict.keys():
        ...
        for value in new_dict[topic].keys():
            if new_dict[topic][value]['mu'] > new_dict[topic][max_value]['mu']:
                max_value = value
...
def most_important_value_topic_negative(new_dict):
    max_value_dict = {}
    for topic in new_dict.keys():
        max_value = 0
        max_mu = 0
...
def topic_summary_negative(annotated_dict):
    return most_important_value_topic_negative(dist_sum_per_value(annotated_dict))
...
def player_summary_negative(learner):
    summary = topic_summary_negative(annotate_learner(learner))
    return [list(summary.keys()), list(summary.values())]